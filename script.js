...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Expose internals so research.js can reuse the same fetch/mapping logic
    // without duplicating it (used for multi-symbol watchlists, CSV export, etc).
    window.__orderbookInternals = {
        symbolMappings,
        symbols,
        getSliceSize: () => parseInt(document.getElementById('slice-size').value)
    };
    ...
    async function fetchData() {
        ...
        if (typeof window.updateComparisonTable === 'function') {
            window.updateComparisonTable(data, selectedSymbol);
        }
        if (typeof window.checkPriceAlerts === 'function') {
            window.checkPriceAlerts(data, selectedSymbol);
        }
        if (typeof window.autoSaveConfig === 'function') {
            window.autoSaveConfig();
        }
        window.__orderbookInternals.lastData = data;
        window.__orderbookInternals.lastSymbol = selectedSymbol;
        window.__orderbookInternals.lastExchanges = selectedExchanges;
        if (typeof window.onOrderbookFetched === 'function') {
            window.onOrderbookFetched(data, selectedSymbol);
        }
    ...
    window.__orderbookInternals.fetchExchangeData = fetchExchangeData;
